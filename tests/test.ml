...
let () =
  ...
  Switch.on_release sw (fun () -> Virtio_gpu.close gpu);
  let transport = Virtio_gpu.connect_wayland ~sw gpu in
  let display, conn_closed = Wayland.Client.connect ~sw transport in
  Fiber.fork ~sw (fun () -> Promise.await_exn conn_closed);
  ...
  let rec animate () =
    Fiber.both
      (fun () -> Eio.Condition.await_no_mutex trigger)
      (fun () ->
    ...
    animate ()
  in
  Fiber.fork_daemon ~sw animate;
  Promise.await closed;
  clear_recycle t;
  Wayland.Client.stop display;
  Promise.await_exn conn_closed
