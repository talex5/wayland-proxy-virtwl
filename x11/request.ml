...
let send_aux (t:Types.display) ~major ?(minor=0) size fn =
  ...
  let reply, set_reply = Lwt.wait () in
  Queue.add ((t.next_seq land 0xffff), set_reply) t.pending;
  let+ () = Lwt_cstruct.(complete (write t.socket) msg) in
...
let send_checked (t:Types.display) ~major ?minor size fn =
  t.need_sync <- true;
  Lwt_condition.broadcast t.wake_input ();
  let* reply = send_aux t ~major ?minor size fn in
  reply >|= function
...
let send_only (t:Types.display) ~major ?minor size fn =
  ...
  Lwt.on_any r
    ...
    (fun ex ->
       Log.err (fun f -> f "Uncaught error in send_only: %a" Fmt.exn ex)
    )

let send_maybe_checked (type a) (checked : a checked) t ~major ?minor size fn : a Lwt.t =
  match checked with
...
let send_exn (t:Types.display) ~major ?minor size fn =
  let n = t.next_seq in
  send t ~major ?minor size fn >|= function
  | `Reply r -> r
...
let send_sync (t:Types.display) ~major ?minor size fn =
  ...
  Lwt.on_any r
    ...
    (fun ex -> Log.err (fun f -> f "Error from sync! %a" Fmt.exn ex))
