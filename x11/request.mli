...
val send_only : Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> unit
...
val send : Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> Types.reply
(** Send an X11 message and wait for the reply. *)

val send_checked : Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> (unit, Error.code) result
(** Send an X11 message that doesn't have a reply, and wait until we know it succeeded. *)

val send_maybe_checked : 'a checked -> Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> 'a
(** [send_maybe_checked Checked] is [send_checked].
    [send_maybe_checked Unchecked] is [send_only]. *)

val send_exn : Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> Cstruct.t
(** Send an X11 message and wait for the reply. Raise an exception if we get an error. *)

val send_sync : Types.display -> major:int -> ?minor:int -> int -> (Cstruct.t -> unit) -> unit
...
val send_event : Types.display -> window:int32 -> propagate:bool -> event_mask:int32 -> (Cstruct.t -> unit) -> unit
