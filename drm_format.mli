...
(** Validate the that the provided arguments (from [Wl_shm_pool.create_buffer])
    ...
    [fd] must be a valid file descriptor.  Otherwise, it is safe to pass
    untrusted input to this function.  The parameters are labeled [untrusted_*]
    to indicate this.

    @return [true] on success or [false] on failure.

    @param fd The file descriptor, see [Wl_shm_pool.create_buffer]
